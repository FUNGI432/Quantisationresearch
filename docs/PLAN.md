...
observable (first QAT-strategy run enabling error tracking; first resume
after Day 6's fixes). (1) Per-layer quantization-error logging called
`.item()` 1,344 times/step (168 layers x 8 microbatches), each a blocking
CUDA sync -- turned one step into ~54 min; fixed by batching to once/step.
(2) Resume's `torch.load(..., map_location=DEVICE)` left a redundant
~2.78GB checkpoint copy in VRAM for the whole run; fixed with an explicit
`del`+`empty_cache()`. Also: `weights_only`'s two completed seeds show a
large, unexplained PPL discrepancy (18.69 vs. 28.91) -- open question, not
yet resolved.
